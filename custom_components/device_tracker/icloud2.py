...
CONF_FILTERDEVICES = 'filter_devices'
CONF_IGNORED_DEVICES = 'ignored_devices'
CONF_ACCOUNTNAME = 'account_name'

# entity attributes
ATTR_ACCOUNTNAME = 'account_name'
ATTR_FILTERDEVICES = 'filter_devices'
...
SERVICE_SCHEMA = vol.Schema({
    ...
    vol.Optional(ATTR_FILTERDEVICES): cv.slugify,
...
def setup_scanner(hass, config: dict, see, discovery_info=None):
    ...
    filter_devices = config.get(CONF_FILTERDEVICES) 
    account = config.get(CONF_ACCOUNTNAME, slugify(username.partition('@')[0]))

    icloudaccount = Icloud(hass, username, password, account, see, filter_devices)
